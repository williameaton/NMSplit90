...
models = ["prem_ani_att_database/model", "1066a/1066a_mineos_model"]

modelsuffix = ["prem", "1066a"]

fig, ax = plt.subplots(3)


for i in range(2): 
    mineos_model = np.loadtxt(f"../../databases/{models[i]}")
    ...
    # 1066a
        ...
        disc = np.array([0, 33, 66, 131, 140, 157, 160])
    ...
    for idisc in disc:
        ...
        discctr += 1
    np.savetxt(fname=f"epsi_discontinuities_{modelsuffix[i]}.txt", X=disc_epsi)
    ...
    np.savetxt(X=etainterp, fname=f'eta_interpolated_{modelsuffix[i]}.txt')
    np.savetxt(X=epsinterp, fname=f'epsilon_interpolated_{modelsuffix[i]}.txt')
