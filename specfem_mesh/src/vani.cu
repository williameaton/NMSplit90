...
__global__ void vanikernel(int size, double *etaptr){ 
  printf("%i %i %f \n", blockIdx.x, threadIdx.x, etaptr[blockIdx.x] );
}
...
__global__ void project_eta_to_gll(int npoints, int ngll, int nspec,
    ...
    // ! convert this i,j,k,nspec to ngll^3 x nspec and store:
    // d_Cxyz_g(ispec, ngll*ngll*(igllx-1) + ngll*(iglly-1) + igllz,  :, :) = Crot
...
extern "C" {
 ...
 int cpp_project_eta_to_gll(int model_npoints, int nspec, int ngll, double modelv[5]){
  ...
  //printf("Ngll per warp: %i \n", ngll_per_warp);
  printf("Nspec: %i \n", nspec);
  //printf("Warps for GLL: %i \n", ngllwarps_per_block);
  //printf("ACLNF: %f  %f  %f  %f  %f \n", modelv[0], modelv[1], modelv[2], modelv[3], modelv[4]);
