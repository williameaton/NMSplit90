module PREMModel
    ...
    subroutine get_PREM_ACLNF_at_centre(A, C, L, N, F)
        ...
        rho = (13.0885d0)*1000.d0/RHOAV
        ! Originally km/s --> m/s 
        vp  = (11.2622d0)*1000.d0/SCALE_V
        vs  = (3.6678d0)*1000.d0/SCALE_V
    ...
    end subroutine get_PREM_ACLNF_at_centre
    ...
    subroutine get_PREM_ACLNF_at_radius(radius, A, C, L, N, F)
        ...
        if(radius.gt.RIC+ 1.0e-5)then 
            write(*,'(a, f12.6, a, f12.6)')'WARNING in get_PREM_ACLNF_at_radius: Only setup for IC but using radius ', radius, ' when maximum is ', RIC
            !stop
        else 
