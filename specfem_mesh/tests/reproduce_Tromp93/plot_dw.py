...
for imode in range(nmodes): 
    ...
    Hmat = np.loadtxt(f"../../ellipticity/matrices/RotEll_{name}_{name}.txt")[:tl1, :]

    # a b c parameters that define the quartic of m 
    abc = np.loadtxt(f"../../ellipticity/abcparams/abc{name}_{name}.txt")
    ...
    rotell = (df + freq)*1000
    rotellcont = (dfcont + freq)*1000

    proxy = [np.NaN, np.NaN]
    ax.axhline(freq*1000, color='purple')
    omegaline, = ax.plot([np.NaN,np.NaN], proxy, '-', color='purple' )
    leglines.append(omegaline)


    ax.plot(mcont, rotellcont, ':k')
    ax.plot(m, rotell, 'xk')
    rotline, = ax.plot([np.NaN,np.NaN], proxy, '-xk' )
    leglines.append(rotline)


    idir = 0 
    for ddir in ['matrices', 'alt_matrices']:
        # Vani contribution
        vani = np.loadtxt(f"{ddir}/vani{name}.txt")[:tl1, :]

        vani = np.diag(vani)/(2*omega0nondim)

        # Now we scale it 
        vani = vani/(SCALE_T*(2*np.pi))

        # Convert to mHz: 
        vani *= 1e3

        # We can compute the quartic polynomial contribution from vani: 
        coeffs = np.polyfit(m, vani, 4)  # Fit a quartic (degree 4) polynomial
        Vani   = np.poly1d(coeffs) 
        vanicont = Vani(mcont)

        vanirotell = rotell + vani
        vanirotellcont = rotellcont+vanicont
        ax.plot(np.arange(-l, l+1), vanirotell, 'o', color=clrs[idir], markersize=4)
        ax.plot(mcont, vanirotellcont, '-', color=clrs[idir])
        lprox, = ax.plot(proxy, proxy, '-o', color=clrs[idir])

        if imode==0: 
            leglines.append(lprox)
    ...
    #ax.legend(leglines, [r"$\omega_0$", 'Rotation', 'A=0, N=0', 'C=0, L=0'])
...
fig.savefig(f'Tromp93_rot_ell_noleg.pdf', format='pdf')